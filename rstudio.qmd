...
1. Start an interactive session: `bsub -q damrauer_interactive -Is bash`


1. Load the singularity module: `module load singularity`

1. Use the `singularity pull` command to download the image from the Github Container Repository: `singularity pull oras://ghcr.io/mglev1n/bioconductor-tidyverse:singularity-latest`

The `singularity pull` command can more broadly be used to download docker images and convert them to singularity containers: `singularity pull docker://ghcr.io/rocker-org/tidyverse:latest`
